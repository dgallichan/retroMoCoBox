...
if size(data,3)==1 || length(size(data))>4
    data = squeeze(data);
end
...
if bSep(1) < 0 % if bSep is less than zero then use this number to pad the images
    if length(bSep)==1
        bSep = [bSep 0];
    end
    data(end+1:end-2*bSep(1),:,:,:) = bSep(2); % extends the image!
    data(:,end+1:end-2*bSep(1),:,:) = bSep(2); % extends the image!
    data = circshift(data,[-bSep(1) -bSep(1) 0 0]);
end
    

[nx ny nz nt] = size(data);

if bSep(1)>0 && size(data,3)>1
    figure
    dims = size(data);
...
%             noCols = round(sqrt(dims(3)));
    ...
    outIm = -1; % no outIm for separate subplots
