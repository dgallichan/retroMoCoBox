function P = spm12_realign_parfor(P,flags)
...
%
% -- 21/4/16 - daniel.gallichan@epfl.ch - changed to use parfor - does not
% allow register-to-mean option
...
parfor i=2:numel(P)
    Ptemp(i) = P(i);
    V  = smooth_vol(Ptemp(i),flags.interp,flags.wrap,flags.fwhm);
    d  = [size(V) 1 1];
    d  = d(1:3);
    ss = Inf;
    countdown = -1;
    for iter=1:64
        [y1,y2,y3] = coords([0 0 0  0 0 0],P1mat,Ptemp(i).mat,x1,x2,x3);
        msk        = find((y1>=1 & y1<=d(1) & y2>=1 & y2<=d(2) & y3>=1 & y3<=d(3)));
        if length(msk)<32, error_message(Ptemp(i)); end
        ...
        p(lkp)     = p(lkp) + soln';
        Ptemp(i).mat   = spm_matrix(p) \ Ptemp(i).mat;
...
disp('You asked to register to the mean image, but the modified parfor version of spm_realign currently doesn''t allow that'); 
