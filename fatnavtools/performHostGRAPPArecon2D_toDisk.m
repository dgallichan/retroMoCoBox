function [timingReport, tempNameRoots] = performHostGRAPPArecon2D_toDisk(twix_obj,tempDir,counterStruct, nSliceNeighbours, combinePars)
%
% function [timingReport, tempNameRoots] = performHostGRAPPArecon2D_toDisk(twix_obj,tempDir,counterStruct, nSliceNeighbours, combinePars)
%
% Applies GRAPPA to the host data, which here is assumed to have acceleration in
% the both PE directions 
%
...
% writes files to disk. Currently the most likely RAM bottleneck is if the
% ACS data won't fit into memory - which could also be handled differently
% in this code if necessary, but I'm hoping for now that is not too big an
% issue in reality.
%
% -------
...
% 
% -- March 2018 - adapt for 2D GRAPPA
...
tempFiles = dir([tempDir '/tempGRAPPAReconData_' MIDstr '_slicedata_*.bin']);
...
if length(tempFiles)~=nread % Check if we can assume they already exist...
    ...
    for iS = 1:nread
        fileid(iS) = fopen([tempDir '/tempGRAPPAReconData_' MIDstr '_slicedata_' num2str(iS,'%.4d') '.bin'],'w');
    end
...
% ACSdata = ifft1s(ACSdata); % even this requires making a copy of the ACS data, which could be quite large for multi-echo GRE
for iEco = 1:nEco
    ACSdata(:,:,:,:,1,1,1,iEco,1,:) =  ifft1s(ACSdata(:,:,:,:,1,1,1,iEco,1,:));
end
clear thisData;

% create a matfile for the ACSdata variable, as this can get quite large
% for multi-echo GRE, and needs to still be in memory for each of the
% parfor loop elements - if we want to be able to use 'nSliceNeighbours' >
% 0
ACSdata_matfileName = [tempDir '/tempGRAPPAReconData_' MIDstr '_ACSdata_matfile'];
save(ACSdata_matfileName,'ACSdata','-v7.3');
clear ACSdata
ACSdata_matfile = matfile(ACSdata_matfileName); 
...
parfor iS = 1:nread
    ...
    for iSet = 1:nSet
        ...
        for iEco = 1:nEco
            ...
            this_fileid = fopen([tempDir '/tempGRAPPAReconData_' MIDstr '_slicedata_' num2str(iS,'%.4d') '.bin'],'r');
            ...
            oneLineX = any(thisdata(:,:,1),2);
            oneLineY = any(thisdata(:,:,1),1);
            startX = find(oneLineX,1,'first'); 
            startY = find(oneLineY,1,'first');  
            ...
            if nSliceNeighbours > 0
                ...
                for this_iS = iS-nSliceNeighbours:iS+nSliceNeighbours
                    if this_iS >= 1 && this_iS <= nread
                        thisACSdataFull = squeeze(ACSdata_matfile.ACSdata(this_iS,:,:,:,1,1,1,iEco,1,iSet));
                ...
                thisACSdataFull = squeeze(ACSdata_matfile.ACSdata(iS,:,:,:,1,1,1,iEco,1,iSet));
                thisACSdataFull = permute(thisACSdataFull,[2 3 1]);
            ...
            thisGrapRecon = GrappaReco3D_arb(thisdata,grapKernel,thisGrapW,[Ax Ay],[startX startY]);
...
delete([ACSdata_matfileName '.mat']);
delete([tempDir '/tempGRAPPAReconData_' MIDstr '_slicedata_*.bin'])
