  function x = nufft_adj_single(X, st)
%|function x = nufft_adj_single(X, st)
...
%
% - Daniel Gallichan - February 2015 - just made small changes to try to
%   use 'single' data format as much as possible to save RAM
...
if ~isfield(st, 'interp_table_adj')
	...
	Xk_all = single(feval(st.interp_table_adj, st, X));
end

x = zeros(prod(Kd), Lprod,'single');			% [*Kd *L]
for ll=1:Lprod
