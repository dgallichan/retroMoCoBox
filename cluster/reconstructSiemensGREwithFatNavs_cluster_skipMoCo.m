function reconstructSiemensGREwithFatNavs_cluster_skipMoCo(rawDataFile, varargin)
...
fprintf(fidHTML,['<h3><emph>This processing was done without the MoCo part!</emph> </h3>\n']);
...
% make these exist for _skipMoCo version:
fitMats_mm_toApply = []; 
alignIndices = [];
...
fprintf(fid,['matlab -nodisplay -nodesktop -nosplash -r "cluster_runMultiEchoGRE_eachEcho_skipMoCo(''' tempNameRoots.allReconPars ''',${SLURM_ARRAY_TASK_ID});exit;"\n']);
fclose(fid);

% disp('Launching batch job array for applying the motion correction...')
disp('Launching batch job array for doing the recon...')
