...
if [ $# -lt 1 ];then
	echo ""
	echo "Runs GRE with FatNavs recon on the CUBRIC cluster - but this version skips the actual MoCo step"
    echo "to try to get the ASPIRE recon (without MoCo) done much faster."
	echo ""
	echo "Usage: cluster_submitGRErecon_skipMoCo.sh <full_path_to_input_raw_twix_file> <full_path_to_output_root_folder>"
	echo ""
